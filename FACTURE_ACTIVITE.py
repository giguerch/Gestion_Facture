...
class FACTURE_ACTIVITE_LIST:
    ...
    def by_id(self, ID_FACTURE):
        ...
        try:
            ...
            print(e)
        cur = conn.cursor()
        cur.execute("SELECT * FROM FACTURE_ACTIVITE WHERE ID_FACTURE = {}".format(self.ID_FACTURE))
        rows = cur.fetchall()
        if len(rows) > 0:
            for row in rows:
                row = list(row)
                for i,r in enumerate(row, start = 0):
                    if r == None:
                        row[i] = ''
                self.FAL.append(FACTURE_ACTIVITE(row[0], row[1], row[2], row[3], row[4]))   
        self.FAL.sort()
        conn.close()
        ...
        for i,v in enumerate(self.FAL):            
            if v.ID == ID:
                ...
                try:
                    ...
                    print(e)
                cur = conn.cursor()
                cur.execute("DELETE FROM FACTURE_ACTIVITE WHERE ID = {}".format(ID))
                conn.commit()
                conn.close()    
    def print(self):
